...
class maxEnt:
    ...
    def calcEp_xy(self):
        ...
        for feature in range(self.featureNum):
            ...
            for (x, y) in self.fixy[feature]:
                ...
                #将计算得到的Ep_xy写入对应的位置中
                #fixy中存放所有对 在训练集中出现过的次数，处于训练集总长度N就是概率了
    ...
    def calcPwy_x(self, X, y):
        ...
        for i in range(self.featureNum):
            ...
            #才是完整的分母，即z = z + numerator  (z要关于y求和)
    ...
    def maxEntropyTrain(self, iter = 500):
        ...
        for i in range(iter):
            ...
            for j in range(self.n):
                ...
                deltaList[j] = (1 / self.M) * np.log(self.Ep_xy[j] / Epxy[j])
            
            #按照算法6.1步骤二中的（b）更新w
            self.w = [self.w[i] + deltaList[i] for i in range(self.n)]
...
if __name__ == '__main__':
    ...
    trainData, trainLabel = loadData('D:\\WORKSPACE\\statistic-machine-learning\\Mnist\\mnist_train.csv')

    # 获取测试集及标签
    print('start read testSet')
    testData, testLabel = loadData('D:\\WORKSPACE\\statistic-machine-learning\\Mnist\\mnist_test.csv')
